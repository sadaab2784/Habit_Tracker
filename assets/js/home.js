  ...
  for(let j=0;j<habitsWeekdays.length;j++) //iterating on all habits elements
  {
    for(let i=0;i<7;i++){//iterating on all days of week
      ...
      if(week[i].substring(8,10) == today){
        ...
        day[0].style.cssText = 'height:100%; width:16%; margin-top:0; box-shadow: 0px 0px 4px 1px #a99cbb';
        let dayName = day[0].getElementsByClassName('day-name');
        dayName[0].style.cssText = 'background:orange;font-size:1rem;'
        let dateElement = day[0].getElementsByClassName('date');
        dateElement[0].innerText = monthName + "," + week[i].substring(8,10);
        dateElement[0].style.cssText = 'font-size:0.9rem; background:orange;';

        //if status 'unfinished' is found
        try{
          let unFinishedElement = day[0].getElementsByClassName('unfinished');
          unFinishedElement[0].style.cssText = 'font-size:1.1rem; margin-top:5px;';
        }catch(err){
          if(err){
            console.log(err);
          }
        ...
        try{
          ...
          finishedElement[0].style.cssText = 'font-size:1.1rem; margin-top:5px;';
        }catch(err){
          if(err){
            console.log(err);
          }
